using UnityEngine;
using System.Collections;

public class Ball : MonoBehaviour {
	
	private Paddle paddle;
	private bool hasStarted = false;
	private Vector3 paddleToBallVector;

	// Use this for initialization
	void Start () {
		paddle = GameObject.FindObjectOfType<Paddle>();
		paddleToBallVector = this.transform.position - paddle.transform.position;
	}
	
	// Update is called once per frame
	void Update () {
		if (!hasStarted) {
			// Lock the ball relative to the paddle.
			this.transform.position = paddle.transform.position + paddleToBallVector;
			
			// Wait for a mouse press to launch.
			if (Input.GetMouseButtonDown(0)) {
				print ("Mouse clicked, launch ball");
				EventManager.LogEvent(Time.time, "Ball", "Movement", "StartBall", "PositionX", transform.position.x);
				EventManager.LogEvent(Time.time, "Ball", "Movement", "StartBall", "PositionY", transform.position.y);
				hasStarted = true;
				this.GetComponent<Rigidbody2D>().velocity = new Vector2 (2f, 10f);
				EventManager.LogEvent(Time.time, "Ball", "Movement", "BallBounce", "VelocityX", GetComponent<Rigidbody2D>().velocity.x);
				EventManager.LogEvent(Time.time, "Ball", "Movement", "BallBounce", "PositionY", GetComponent<Rigidbody2D>().velocity.y);
			}
		}
		EventManager.LogEvent(Time.time, "Ball", "Movement", "BallPosition", "PositionX", transform.position.x);
		EventManager.LogEvent(Time.time, "Ball", "Movement", "BallPosition", "PositionY", transform.position.y);
	}
	
	void OnCollisionEnter2D (Collision2D collision) {
		// Ball does not trigger sound when brick is destoyed.
		// Not 100% sure why, possibly because brick isn't there.
		Vector2 tweak = new Vector2 (Random.Range(0f, 0.2f), Random.Range(0f, 0.2f));
		
		if (hasStarted) {	
//			GetComponent<AudioSource>().Play();
			EventManager.LogEvent(Time.time, "Ball", "Movement", "BallBounce", "PositionX", transform.position.x);
			EventManager.LogEvent(Time.time, "Ball", "Movement", "BallBounce", "PositionY", transform.position.y);
			GetComponent<Rigidbody2D>().velocity += tweak;
			EventManager.LogEvent(Time.time, "Ball", "Movement", "BallBounce", "VelocityX", GetComponent<Rigidbody2D>().velocity.x);
			EventManager.LogEvent(Time.time, "Ball", "Movement", "BallBounce", "VelocityY", GetComponent<Rigidbody2D>().velocity.y);
		}
	}
}
